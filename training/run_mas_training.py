...
from training.train_mas import train_mas
...
def plot_substance_accuracies(results_over_time, N_RUNS, save_path="MAS_baseline_performance.png"):
    substances = [0, 1, 2, 3]
    ...
    plt.title(f"Accuracy per Substance Over Time for MAS averaged over {N_RUNS} runs")
    plt.legend()
    plt.grid(axis="y", linestyle="--", alpha=0.5)
    plt.savefig(save_path, dpi=200)
    plt.close()

    print(f"Saved MAS baseline performance plot to {save_path}")
...
if __name__ == "__main__":
    ...
    # -----------------------------
    # Run MAS training N times
    # -----------------------------
    for i in tqdm(range(N_RUNS)):

        model, results, results_over_time = train_mas(
